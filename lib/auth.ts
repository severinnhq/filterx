...
export async function createUser(body: { email: string; password: string }) {
  ...
  const result = await db.collection("users").insertOne({
    ...
    status: "free", // Set initial status to "free"
  })
...
export async function updateUserStatus(userId: string, status: string) {
  ...
  const result = await db.collection("users").updateOne({ _id: new ObjectId(userId) }, { $set: { status: status } })
...
export async function updateUserEmail(userId: string, email: string) {
  ...
  const result = await db.collection("users").updateOne({ _id: new ObjectId(userId) }, { $set: { email: email } })
